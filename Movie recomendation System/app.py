...
def fetch_poster_b64(title):
    try:
        ...
        if not results:
            ...
            r = requests.get(f"https://www.omdbapi.com/?t={requests.utils.quote(title)}&type=movie&apikey={OMDB_KEY}",
                             timeout=6)
            data = r.json()
    ...
    for t in titles:
        try:
            ...
            if results and results[0].get("Poster") and results[0]["Poster"] != "N/A":
                ...
                r2 = requests.get(f"https://www.omdbapi.com/?t={requests.utils.quote(t)}&type=movie&apikey={OMDB_KEY}",
                                  timeout=5)
...
st.markdown('<div class="foot">CineMatch &nbsp;·&nbsp; AI Recommendation Engine &nbsp;·&nbsp; 2025</div>',
            unsafe_allow_html=True)